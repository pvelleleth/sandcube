...
Go tests additionally exercise hostile concurrent symlink replacement, special files/FIFOs, hardlink-safe replacement, file modes, bounded output under concurrent writes, process routing/validation and cross-sandbox lookup. Crystal specs check authentication/routing, binary transport, decoded traversal, and body limits. Tests run with Go's race detector and vet. PostgreSQL specs use `TEST_DATABASE_URL` or fall back to `DATABASE_URL`, with a temporary schema for each example. The database role needs permission to create schemas. They are pending only when neither variable is set.
