...
Create reserves its image and records configuration and intent in one PostgreSQL transaction. Start, stop, restart and delete also commit intent before calling the adapter. A host-wide database lock serializes lifecycle operations across API instances. The adapter serializes operations per sandbox, including process preparation and file access. Synchronous exec releases that lock while waiting, so stop/delete can interrupt it.
