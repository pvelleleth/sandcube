...
module Sandcube
  ...
  class ImageStore
    def initialize(@db : DB::Database)
      {{ read_file("#{__DIR__}/../../migrations/001_images.sql") }}.split(';').each do |statement|
        @db.exec(statement) unless statement.strip.empty?
      end
    end

    def get(id : String) : JSON::Any
      raw = @db.query_one?("SELECT row_to_json(images)::text FROM images WHERE id=$1", id, as: String)
    ...
    def ready(id, digest)
      @db.exec("UPDATE images SET status='READY',oci_digest=$2,error_message=NULL,updated_at=now() WHERE id=$1 AND status='BUILDING'", id, digest)
    end

    def failed(id, message)
      @db.exec("UPDATE images SET status='ERROR',error_message=$2,updated_at=now() WHERE id=$1", id, message)
    end
    ...
    def reserve(id, sandbox_id) : String
      ...
      @db.transaction do |tx|
        conn = tx.connection
        row = conn.query_one?("SELECT status,oci_reference FROM images WHERE id=$1 FOR UPDATE", id, as: {String, String})
    ...
    def begin_delete(id) : JSON::Any
      @db.transaction do |tx|
        conn = tx.connection
        state = conn.query_one?("SELECT status FROM images WHERE id=$1 FOR UPDATE", id, as: String)
        ...
        conn.exec("UPDATE images SET status='DELETING',updated_at=now() WHERE id=$1 AND status != 'DELETED'", id)
      end
      get(id)
    end

    def deleted(id)
      @db.exec("UPDATE images SET status='DELETED',updated_at=now(),error_message=NULL WHERE id=$1", id)
      get(id)
    end

    def interrupted_builds
      @db.exec("UPDATE images SET status='ERROR',error_message='Build interrupted by service restart',updated_at=now() WHERE status='BUILDING'")
    end
  end
end
